...
impl Guest for Component {
    fn page(edgee_event: Event, settings: Dict) -> Result<EdgeeRequest, String> {
        send_to_firehose(edgee_event, settings)
    }
...
fn send_to_firehose(edgee_event: Event, settings_dict: Dict) -> Result<EdgeeRequest, String> {
    let firehose_settings =
        firehose_payload::Settings::new(settings_dict).map_err(|e| e.to_string())?;
    ...
    let url = firehose_settings.generate_firehose_url();
    let body = firehose_settings.generate_body(record.clone());
    let headers = firehose_settings.generate_headers(url.clone(), body.clone());
...
mod tests {
    ...
    fn page_works_fine() {
        ...
        let settings = vec![
            ...
            ("firehose_stream".to_string(), "stream-name".to_string()),
        ];
        ...
        assert_eq!(
            edgee_request
                .url
                .starts_with("https://firehose.eu-west-1.amazonaws.com/"),
            true
        );
